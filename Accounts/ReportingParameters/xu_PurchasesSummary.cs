...
{
    public partial class xu_PurchasesSummary : DevExpress.XtraEditors.XtraUserControl
    {
        public xu_PurchasesSummary()
        {
            InitializeComponent();
        }
        ...
        private void xu_PurchasesSummary_Load(object sender, EventArgs e)
        {
        ...
        {
            Reports.Accounts_PurchasesSummary Ledger = new Reports.Accounts_PurchasesSummary();
            ...
            string query = "";
            Ledger.BeginInit();
            query = "SELECT     iType,purchaseTypename,AccountID,AccountName, ROUND(SUM(Qty), 2) AS tQTY, SUM(ValExTax) AS tValExTax, SUM(ValTax) AS tValTax, SUM(ValInTax) " +
                  "    AS tValInTax, SUM(ValWHT) AS tValWHT FROM  dbo.Accounts_PurchasesSummary_IstStage where iDate>='" + FromDate.DateTime.ToString(MachineEyes.Classes.Accounting.culture.DateTimeFormat.ShortDatePattern) + "' and iDate<='" + this.ToDate.DateTime.ToString(MachineEyes.Classes.Accounting.culture.DateTimeFormat.ShortDatePattern) + "' and iCat in(" + VCat + ") GROUP BY iType,purchaseTypename,AccountID,AccountName ";
