...
{
    public partial class Data_Article_Jacquard : DevExpress.XtraEditors.XtraForm
    {
       
        enum UserInputMode { View = 0, Add = 1, Edit = 2, Delete = 3 };
        UserInputMode uiMode;
        public Data_Article_Jacquard()
        {
            InitializeComponent();
        }
        ...
        private void Update_Article()
        {
            ...
            for (int x = 0; x < this.scroll_Fancy.Controls.Count; x++)
            {
                ...
                UserControls.dxGreigeArticleParams dytb = (UserControls.dxGreigeArticleParams)this.scroll_Fancy.Controls[x];
                if (dytb.Counts.EditValue == null)
                {
                    XtraMessageBox.Show("invalid Count", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
            ...
            for (int x = 0; x < this.scroll_Fancy.Controls.Count; x++)
            {
                ...
                if (dytb.Weight.EditValue != null)
                    query += ",FancyWeight_" + x.ToString() + "=" + (string)dytb.Weight.EditValue + "";
                else
                    query += ",FancyWeight_" + x.ToString() + "=null";
                if (dytb.Ply.EditValue != null)
        ...
        private void Save_Article()
        {
            ...
            //execute save query (save records into database)
            string query = "insert into PP_Article (articlenumber,articletypeid,articlename,ArticleShortName,FabricName,EPI,PPI,Width,InsertionType,ArticleWeave,ArticleSelvage";
            ...
            for (int x = 0; x < this.scroll_Fancy.Controls.Count; x++)
            {
                query += ",fancyyarncount_" + x.ToString() + ",fancyweight_" + x.ToString() + ",fancyyarnply_" + x.ToString() + ",fancyyarnblend_" + x.ToString() + ",fancyyarndesc_" + x.ToString() + ",fancyyarncolor_" + x.ToString() + ",fancyyarnpicks_" + x.ToString();


            }
            query += ") values (";

            query += "'" + this.Article_Number_textEdit.Text + "',2,'" + this.Article_Details_Label.Text + "','"+this.ShortName.Text +"'";
            if (this.FabricName.EditValue != null)
        ...
        private void Clear_Article()
        {
           ...
           dxy.Ply.EditValue = null;
           dxy.PicksorEnds.Visible = false;
           dxy = (UserControls.dxGreigeArticleParams)this.scroll_Fancy.Controls[0];
           dxy.Counts.EditValue = null;
           dxy.Blends.EditValue = null;
           dxy.Color.EditValue = null;
           dxy.Desc.EditValue = null;
        ...
        {
            UserControls.dxGreigeArticleParams fancyparam = new UserControls.dxGreigeArticleParams();
            fancyparam.isWarp = false;
            double dPPI = 0, dEPI = 0, dWidth = 0;
            if (this.Article_PPI_textEdit.EditValue != null)
                double.TryParse(this.Article_PPI_textEdit.EditValue.ToString(), out dPPI);
            if (this.Article_EPI_textEdit.EditValue != null)
            ...
            if (this.scroll_Fancy.Controls.Count <= 0) fancyparam.Top = 0;
                ...
                fancyparam.Top = this.scroll_Fancy.Controls[this.scroll_Fancy.Controls.Count - 1].Top + fancyparam.Height;
        ...
        private void btn_View_Click(object sender, EventArgs e)
        {
            ...
            Program.MainWindow.ArticleJacquardView.ShowDialog();

            try
            {
                if (Program.MainWindow.ArticleJacquardView.SelectedRow.Status != ParameterStatus.Cancelled)
                {
                    Clear_Article();
                    this.Article_Number_textEdit.Tag = Program.MainWindow.ArticleJacquardView.SelectedRow.PrimeryID;
                    this.Article_Number_textEdit.Text = Program.MainWindow.ArticleJacquardView.SelectedRow.PrimeryID;
                    this.Article_Details_Label.Tag = Program.MainWindow.ArticleJacquardView.SelectedRow.PrimaryString;
                    ...
                    try
                    {
                        ...
                        for (int x = 0; x <= 5; x++)
                        {
                            ...
                            {
                                if (x > 0) Add_ArticleFancyParams();
                                dytb = (UserControls.dxGreigeArticleParams)this.scroll_Fancy.Controls[x];
                                if (ds.Tables[0].Rows[0]["FancyYarnCount_" + x.ToString() + ""].ToString() != "")
                                    dytb.Counts.EditValue = ds.Tables[0].Rows[0]["WeftYarnCount_" + x.ToString() + ""].ToString();
                                else
                                    dytb.Counts.EditValue = null;
        ...
        private void Delete_Fancy_Click(object sender, EventArgs e)
        {
            if (this.scroll_Fancy.Controls.Count > 1)
